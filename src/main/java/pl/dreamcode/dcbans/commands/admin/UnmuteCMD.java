...
public class UnmuteCMD extends CCommand {
    public UnmuteCMD() {
        super("unmute");
    }
    ...
    public void run(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(ChatUtil.fixColors(config.getUsage())
                    .replace("%USAGE%", "/unmute [nick] [powod]"));
            return;
        }
        ...
        BanUtils.removeBan(u, BanType.MUTE);
        BanUtils.removeBan(u, BanType.TEMP_MUTE);
        if (config.isBcBans()) {
            ChatUtil.broadcastMessage(ChatUtil.fixColors(config.getBcUnmute())
        ...
        if(config.isBcAdmin()) sender.sendMessage(ChatUtil.fixColors(config.getUnmute()));
    }
}
